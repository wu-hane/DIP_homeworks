...
def create_mask_from_points(points, img_h, img_w):
    ...
    draw_points = [(point[0] , point[1]) for point in points]
    draw.polygon(draw_points , fill=255)
    mask = np.array(mask)
    return mask
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    foreground_mask = foreground_mask.expand_as(foreground_img)
    background_mask = background_mask.expand_as(blended_img)

    laplacian_kernel = torch.tensor([
        [[0., 1., 0.], [1., -4., 1.], [0., 1., 0.]],
        [[0., 1., 0.], [1., -4., 1.], [0., 1., 0.]],
        [[0., 1., 0.], [1., -4., 1.], [0., 1., 0.]]
        ]).unsqueeze(0).type_as(foreground_img)


    foreground_laplace = F.conv2d(foreground_img, laplacian_kernel, padding=1)
    blended_laplace = F.conv2d(blended_img, laplacian_kernel, padding=1)
    
    maxsize1 = max(foreground_laplace.size(2),blended_laplace.size(2))
    maxsize2 = max(foreground_laplace.size(3),blended_laplace.size(3))
    device = 'cuda:0' if torch.cuda.is_available() else 'cpu'
    reget_mask = torch.zeros(1,3,maxsize1,maxsize2).to(device)
    c_foreground = reget_mask.clone()
    c_background = reget_mask.clone()
    c_foreground[:, :, :foreground_laplace.size(2), :foreground_laplace.size(3)] = foreground_mask[:, :, :foreground_laplace.size(2), :foreground_laplace.size(3)]
    c_background[:, :, :blended_laplace.size(2), :blended_laplace.size(3)] = background_mask[:, :, :blended_laplace.size(2), :blended_laplace.size(3)]

    reget_lap = torch.zeros(1,1,maxsize1,maxsize2).to(device)
    c_foreground_laplace = reget_lap.clone()
    c_foreground_laplace[:,:,:foreground_laplace.size(2), :foreground_laplace.size(3)] = foreground_laplace
    c_background_laplace = reget_lap.clone()
    c_background_laplace[:, :, :blended_laplace.size(2), :blended_laplace.size(3)] =blended_laplace
    
    laplacian_f =  (c_foreground_laplace)*c_foreground
    laplacian_g =  (c_background_laplace)*c_background

    loss = torch.sum(torch.pow((laplacian_f-laplacian_g),2))
    return loss
...
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state):
    ...
    result = torch.clamp(blended_img.detach(), 0, 1).cpu()
    
    # 检查是否有 NaN 或无穷大值
    assert not torch.isnan(result).any(), "Invalid values encountered"
    assert not torch.isinf(result).any(), "Invalid values encountered"

    # 使用 nan_to_num 处理可能出现的 NaN 或无穷大值
    result = torch.nan_to_num(result)

    # 将张量转换为 [0, 255] 范围内的 uint8 类型
    result = (result * 255).byte().permute(0, 2, 3, 1).squeeze().numpy()
